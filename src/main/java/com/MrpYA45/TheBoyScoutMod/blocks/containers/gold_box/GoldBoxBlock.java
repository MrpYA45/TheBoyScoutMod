...
public class GoldBoxBlock extends BlockBase implements ITileEntityProvider {

	public GoldBoxBlock(String name) {
		super(name, Material.WOOD, 2.0F, 15.0F, SoundType.WOOD);
	}
	
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return Item.getItemFromBlock(ModBlocks.GOLD_BOX_BLOCK);
	}
	
	public ItemStack getItem(World worldIn, BlockPos pos, IBlockState state) {
		return new ItemStack(ModBlocks.GOLD_BOX_BLOCK);
	}
	
	@Override
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		GoldBoxTileEntity te = (GoldBoxTileEntity) worldIn.getTileEntity(pos);
		if(!worldIn.isRemote && te != null && te.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, EnumFacing.NORTH)) {
			playerIn.openGui(Main.instance, Reference.GUI_GOLD_BOX, worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
    ...
    public void breakBlock(World world, BlockPos pos, IBlockState state) {
    	GoldBoxTileEntity te = (GoldBoxTileEntity) world.getTileEntity(pos);
